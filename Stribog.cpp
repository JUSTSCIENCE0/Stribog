...
UINT64 BYTEtoUINT64(BYTE* data, UINT16 start)
{
    UINT64 res = 0;
    for (int i = 0; i < 8; i++)
    {
...
BYTE* UINTSto512bits(UINT64* data)
{
...
BYTE* STransparent(BYTE* data, UINT16 sz)
{
...
{
    UINT64* nums = new UINT64[8];
    for (int n = 7; n >= 0; n--)
    {
        UINT64 tmp = BYTEtoUINT64(data, 8 * n);
        UINT64 mask = 0x8000000000000000;
        nums[n] = 0;
        for (int i = 0; i < 64; i++)
        {
            bool check = (tmp & mask) != 0;
            nums[n] ^= (AMatrix[i] * (int)check);
            mask >>= 1;
        }
    }
    return UINTSto512bits(nums);
}

BYTE* KeySchedule(BYTE* K, UINT8 i)
{
    BYTE* res = new BYTE[64];

    res = XOR(K, C[i], 64);
    res = STransparent(res, 64);
    res = PTransparent(res);
    res = LTransparent(res);
    return res;
}

BYTE* ETransparent(BYTE* K, BYTE* m)
{
    BYTE* res = new BYTE[64];
    res = XOR(K, m, 64);
...
BYTE* Compression(BYTE* N, BYTE* m, BYTE* h)
{
    BYTE* K = XOR(h, N, 64);
    K = STransparent(K, 64);
    K = PTransparent(K);
    K = LTransparent(K);
    BYTE* t = ETransparent(K, m);
    t = XOR(h, t, 64);
    return XOR(t, m, 64);
}
...
int main()
{
    ...
    UINT64* TestM = new UINT64[8];
    TestM[0] = 0x0132313039383736;
    TestM[1] = 0x3534333231303938;
    TestM[2] = 0x3736353433323130;
    TestM[3] = 0x3938373635343332;
    TestM[4] = 0x3130393837363534;
    TestM[5] = 0x3332313039383736;
    TestM[6] = 0x3534333231303938;
    TestM[7] = 0x3736353433323130;

    BYTE* m = UINTSto512bits(TestM);
    for (int i = 0; i < 64; i++)
    {
        N[i] = 0x0; h[i] = 0x0; Sigma[i] = 0x0; Zero[i] = 0x0;
    }
    ...
    h = Compression(Zero, N, h);
    cout << "h = ";
    Print(h);

    h = Compression(Zero, Sigma, h);
    cout << "h = ";
    Print(h);
